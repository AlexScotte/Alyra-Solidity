...
contract VotingV2 is Ownable{
    ...
    function _restartSession() external onlyOwner{
        ...
        require(currentVotingSession == WorkflowStatus.VotesTallied, "The previous session is not over yet.");
    ...
    function voterAddVote(uint16 _proposalId) external onlyRegistered{
        ...
        if(voterProposals[_proposalId].voteCount > voterProposals[winningProposalId].voteCount){
